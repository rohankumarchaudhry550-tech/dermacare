    ...
    <div className="relative min-h-screen py-10 px-6">
      ...
      <div className="max-w-6xl mx-auto space-y-8 relative z-10">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
          ...
          <span>/</span>
          <span className="text-slate-800 font-medium line-clamp-1">{t(article.title)}</span>
        </div>
        ...
        >
          <ArrowLeft className="h-4 w-4" /> {t("Back to Medical Blog")}
        </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <article className="lg:col-span-8 bg-white rounded-3xl p-6 sm:p-10 border border-accent/25 shadow-luxury space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs font-bold text-secondary uppercase tracking-wider">
              <span className="bg-accent/40 px-3 py-1 rounded-full">{t(article.category)}</span>
              <span className="flex items-center gap-1"><Clock className="h-4 w-4" /> {t(article.readTime) || article.readTime}</span>
              <span className="flex items-center gap-1"><Calendar className="h-4 w-4" /> {t(article.publishDate) || article.publishDate}</span>
            </div>
            ...
            <div className="pt-8 border-t border-slate-150 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs">
              <span className="font-bold text-slate-450 uppercase tracking-wider flex items-center gap-1.5">
                <Share2 className="h-4 w-4" /> {t("Share This Medical Guide:")}
              </span>
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-24 space-y-6">
            ...
            <div className="bg-slate-900 text-slate-350 rounded-3xl p-6 border border-slate-850 space-y-4">
              ...
              <div className="flex gap-3 items-center">
                ...
                <div>
                  <h4 className="font-poppins font-bold text-white text-xs sm:text-sm">{t(article.author)}</h4>
                  <p className="text-[10px] text-slate-400 font-semibold">{t("MD - AIIMS | Senior Dermatologist")}</p>
                </div>
              </div>
              <p className="text-[10px] text-slate-400 leading-relaxed border-t border-slate-800 pt-3">
                {t("Dr. Aryan Sharma is committed to providing evidence-based healthcare education, resolving patient doubts via verified journals.")}
              </p>
            </div>
            ...
            {relatedArticles.length > 0 && (
              <div className="bg-white rounded-3xl p-6 border border-accent/30 shadow-luxury space-y-4">
                <h3 className="font-poppins font-bold text-xs sm:text-sm text-slate-850 flex items-center gap-1.5">
                  <Sparkles className="h-4 w-4 text-primary" /> {t("Related Articles")}
                </h3>
