    ...
    <div className="relative min-h-screen py-16 px-6">
      ...
      <div className="max-w-6xl mx-auto space-y-16 relative z-10">
        ...
        <div className="text-center space-y-4 max-w-2xl mx-auto">
          <span className="text-xs font-bold text-secondary uppercase tracking-widest block">{t("Connect With Us")}</span>
          <h1 className="text-4xl sm:text-5xl font-extrabold text-slate-900 font-poppins">
            {t("Contact & Appointments")}
          </h1>
          <p className="text-slate-500 text-xs sm:text-sm leading-relaxed">
            {t("Schedule your premium consultation or reach out to our South Mumbai coordination desk for clinical queries.")}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="bg-white rounded-3xl p-6 border border-accent/20 shadow-sm space-y-5">
              <h3 className="font-poppins font-bold text-base text-slate-800 flex items-center gap-1.5">
                <Sparkles className="h-4.5 w-4.5 text-primary" /> {t("Clinic Coordinates")}
              </h3>
              ...
              <div className="space-y-4 text-xs text-slate-600">
                ...
                <p className="flex items-start gap-3">
                  ...
                  <span>
                    {t("Mon - Sat: 10:00 AM - 07:00 PM")}
                    <br />
                    {t("Sunday: Closed (Prior Booking Only)")}
                  </span>
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-6 border border-accent/20 shadow-sm space-y-4">
              <h3 className="font-poppins font-bold text-base text-slate-800 flex items-center gap-1.5">
                <Car className="h-5 w-5 text-primary" /> {t("Parking & Accessibility")}
              </h3>
              <p className="text-xs text-slate-500 leading-relaxed">
                {t("DermaCare+ clinic offers complimentary secure basement valet parking for all patients. Safe elevator accessibility is available directly from the parking bays to our first-floor suite reception lobby.")}
              </p>
            </div>

            {/* Instant Support options (WhatsApp / Phone) */}
            <div className="bg-white rounded-3xl p-6 border border-accent/25 shadow-sm space-y-4">
              <h3 className="font-poppins font-bold text-base text-slate-800">{t("Instant Chat & Emergency")}</h3>
              ...
              <div className="space-y-3">
                ...
                >
                  <MessageSquare className="h-4.5 w-4.5" /> {t("Chat on WhatsApp")}
                </button>
                ...
                <div className="bg-red-950/20 border border-red-900/25 p-4 rounded-xl flex gap-3 items-start">
                  ...
                  <div>
                    <h5 className="text-xs font-bold text-red-500 uppercase tracking-wider">{t("EMERGENCY SKIN ALLERGY?")}</h5>
                    <p className="text-[10px] text-slate-500 mt-1 leading-relaxed">
                      {t("If you're suffering from acute hives or a sudden blistering eruption, contact our on-duty emergency clinician directly at")} <span className="font-bold text-slate-700">+91 99999 88888</span>.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="text-center sm:text-left mb-2">
              <h2 className="text-xl sm:text-2xl font-bold font-poppins text-slate-900">{t("Schedule Consultation")}</h2>
              <p className="text-slate-500 text-xs mt-1">{t("Select a treatment and your preferred slot to book instantly.")}</p>
            </div>
            <BookingForm />
          </div>
        </div>
